...
def move_zeros(lst):
    x = [x for x in lst if (type(x) is int or type(x) is float) and x != 0]
    y = [x for x in lst if (type(x) is int or type(x) is float) and x == 0]
    lst = x + y
    # еще можно так lst = sorted(lst, key=lambda x: not x)
    # либо так lst.sort(key=bool, reverse=True)
    return lst
